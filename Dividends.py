...
def analysis(file):
    ...
    for line in lines:
        matches = re.findall(
            r'(\d{4})-\d{2}-\d{2}\s*Dividend\s*\$(\d+\.\d+)\s*', line)
...
def create_report(years):
    ...
    worksheet.write(
        'D1', 'Сумма с учётом инфляции (' + str(INF * 100) + '%) на ' +
        str(COUNT_STOCKS) + ' акц.', bold)

    row = 2
    for year in years:
        worksheet.write('A' + str(row), year, center)
        worksheet.write('B' + str(row), '$' + str(round(years[year][0], 2)),
                        center)
